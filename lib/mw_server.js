...
{ 
  ...
  */
  socket.on('newconnection', function(startPacket)
  {
	...
	console.log("New user'" + startPacket[1] + "'is connecting.");
	...
	startPacket[1] = publicName;

    users[id] = startPacket;

	socket.emit('initiate', id, users);

    socket.emit('sceneUpdate', "lamp1", sceneObjects["lamp1"]);
    socket.emit('sceneUpdate', "lamp2", sceneObjects["lamp2"]);	
 ...
 /*
  * Recieved when a client changes their position
  */
  ...
  */ 
  socket.on('environmentChange', function(objectId)
  {
	sceneObjects[objectId] = !sceneObjects[objectId];
      
    io.emit('sceneUpdate', objectId, sceneObjects[objectId]);
  });  
