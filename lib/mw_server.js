...
{ 
  ...
  */
  socket.on('newconnection', function(name, pos, rot, avatar)
  {
    ...
    users[id] = [publicName, pos, rot, avatar];
	...
	*
	*/
	socket.on('updateposition', function(id, name, pos, rot, avatar)
	{
		if (users[id] != undefined) {

			// Update the master list with the client's new location.
			users[id] = [name, pos, rot, avatar];
  ...
  */
  ...
  {
	  io.emit('newmessage', users[id][0], message);
  ...
  */
  socket.on('newavatar', function(id, avatar) {	
	  ...
	  console.log(users[id][0] + "has changed their avatar.");
