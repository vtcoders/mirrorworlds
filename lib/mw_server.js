...
function handler(req, res) 
{
    ...
    switch(pathname) {
        // We define the model "world" service here:
        case '/mw/world':
        case '/mw/world/':
            // We require a URL like: /mw/world?file=model.x3d
            var file = querystring.parse(urL.query).file;
            if ( file == null ) {
                res.writeHead(500);
                res.end('Error getting: ' + req.url);
                console.error('Bad world query no file= given for request: ' + req.url);
                return;
            }
            // TODO: This will not be good for large files, or in general.
            res.writeHead(200);
            fs.readFile(path.join(config.mw_prefix, config.mw_dir_str,
                        'head.htm'), function(err, data) {
                if (err) {
                    res.end('Error getting ' + req.url);
                    console.error(err);
                    return;
                ...
                res.write(data);
                res.write('   <inline url="' + file + '"></inline>\n');
                fs.readFile(path.join(config.mw_prefix, config.mw_dir_str,
                        'foot.htm'), function(err, data) {
                    if (err) {
                        res.end('Error getting ' + req.url);
                        console.error(err);
                        return;
...
{ 
  ...
  */ 
  socket.on('environmentChange', function(eObject) {
      ...
      io.emit('updateEnvironment', environmentEvents[eObject].getName(), environmentEvents[eObject].getState(), environmentEvents[eObject].getType());
  });  
...
http.listen(() => {
    ...
    // If there is no binding address than we will be listening on
    // all Internet interfaces, including localhost (127.0.0.1) if it's
    // setup and usable.
    //
    ...
    if(address.address.toString() === '::') {
        console.error('server listening at: http://localhost:' + address.port);
        console.error('                and: http://' + require('os').hostname() +
                ':' + address.port);
    } else {
