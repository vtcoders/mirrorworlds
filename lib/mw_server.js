...
var path = require('path')
  ...
  , kinectServer = http.Server(express());
  , ioKinect = require('socket.io')(kinectServer)
  , fs = require('fs')
  , url = require('url')
  , querystring = require('querystring');

kinectServer.listen(8181);
...
// Global variable for kinect JSON object
var kinectJson = "";
...
//----------------------------------------------
/**
 * Socket Connection for kinect Json receiving
 *
 * @param socket - the connected socket
 */
ioKinect.on('connection', function (socket)
{
	//----------------------------------------------
	/**
	 * Received when a json emitting client opens a WebSocket connection successfully
	 *
	 * @param startPacket - the array of user information
	 */
	socket.on('newconnection', function(jsonData)
	{
	...
	socket.on('kinectUpdate', function(jsonData)
	{
		console.log("Json streaming successfully");
	...
	socket.on('disconnect', function() {
		...
		console.log("Json streaming ended");
