...
{ 
  ...
  *
  * @param source - type of client connection (web, kinect, blob, etc)
  * @param inUsers - list of connecting users for that source (kinect <= 4)
  */
  socket.on('newconnection', function(source, inUsers)
  {

	//Add a new connection to the server
	console.log("New " + source + "user is connecting.");

	//Give connection a unique ID
	var sourceId = "source" + genId();

	//Make an array to store user data
	var users = {};

	for (i = 0; i < inUsers.length; i++) {

		var userId = "user" + genId();
		...
		users[userId] = inUsers[i];
	}

	//Store users in array of connections
	connections[sourceId] = users;

	socket.username = sourceId;
    
	/**
	* Give connecting client intial update information and unique Id
	* 
	* @param connections - list of connected users
	* @param sourceId - specific ID for this client's connection
	*/
	socket.emit('initiate', connections, sourceId);

	/**
    * Tell everybody else that there is a new connection
	*
	* @param connections[sourceId] - this connection's information
	*/
	socket.broadcast.emit('newuser', connections[sourceId]);
