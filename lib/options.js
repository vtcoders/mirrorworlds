...
exports.parse = (config) =>
{
     ...
     * worlds internal files (the directory where mw/ is) now. */
    if(!fs.existsSync(path.join(config.doc_root, 'mw'))) {

        var mw_prefix = path.join(path.dirname(__dirname), 'public');
        var mw_dir = path.join(mw_prefix, 'mw');

        config.mw_prefix = mw_prefix;
        ...
        // We define '/mw/' ('\mw\' in windows) as a variable so that this
        // code is portable and we do not need to calculate it again at
        // each server request.
