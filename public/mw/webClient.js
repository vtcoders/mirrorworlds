...
var clientInfo = {
	...
	// The source of the client (web, blob, kinect)
	source:"web"

	// The json data of the client (kinect)
	jointData: null;
    ...
    model.onload = function() {
        ...
        socket.on('connect', function() {
            ...
            socket.on('initiate', function(myId, userList) {
                ...
                var jointModelGroup = getElementById("jointModelGroup");
                jointModelGroup.innerHTML = "";
                ...
                for (var userId in userList) {
                    ...
                    if (userId.source == "kinect"){
                    	// Generate a Transform to hold current's joint model
	        		    var userJointModel = document.createElement('Transform');
	              	    userJointModel.setAttribute("translation", "0 -.5 .5");
	              		userJointModel.setAttribute("rotation", "0 0 0 0");
	                   	userJointModel.setAttribute("id", userId + "JointModel");

	                    // Generate an Inline to hold joint model
	                    var characterOfJointModel = document.createElement('inline');
	                    characterOfJointModel.setAttribute("id", userId + "Inline");
	                    characterOfJointModel.setAttribute("url", current.jointModel);

                    	// Add joint model to the avatar Transform
                    	userJointModel.appendChild(characterOfJointModel);
                    }
               		else {
                    ...
                    // If adding self, add to a bundle with camera
                   	if (userId == uniqueId) {
                        ...
                        userBundle.setAttribute("translation", current.position.x + " " +
                   		...
                   		if (userId.source == "kinect"){
                   			...
                   			// Make joint model a child of bundle
                   			userBundle.appendChild(userJointModel);
                   	...
                   	// If adding someone else, add them to the group of other avatars
                    	...
                    	if (userId.source == "kinect"){
                    		jointModelGroup.appendChild(userJointModel);
                   		}
                   		else {
			...
			socket.on('clientUpdate', function(user, id) {
				...
				var bodies = document.getElementById("scene");
				...
				// For kinect users, Update jointModel
				if (user.source == "kinect"){
					if (typeof user.jointData === "string") {
